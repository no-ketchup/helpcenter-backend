...
from app.domain.models import Category as CategoryModel, UserGuide as UserGuideModel, Feedback as FeedbackModel, Media as MediaModel
from app.domain.schema import Category, Media, UserGuide, Feedback

# -------------------------------
# Mapping helpers
# -------------------------------
def to_category(model: CategoryModel, include_guides=True) -> Category:
    return Category(
        id=str(model.id),
        name=model.name,
        ...
        guides=[to_userguide_shallow(g) for g in model.guides] if include_guides else [],
    )
...
def to_userguide(model: UserGuideModel, include_categories=True) -> UserGuide:
    return UserGuide(
        id=str(model.id),
        title=model.title,
        slug=model.slug,
        ...
        createdAt=model.created_at,
        updatedAt=model.updated_at,
        categories=[to_category(c, include_guides=False) for c in model.categories] if include_categories else [],
...
def to_userguide_shallow(model: UserGuideModel) -> UserGuide:
    return to_userguide(model, include_categories=False)
...
class Query:
    ...
    async def categories(self) -> List[Category]:
        async with get_session() as session:
            results = (await session.exec(select(CategoryModel))).all()
            return [to_category(c) for c in results]

    @strawberry.field(description="Fetch a single category by slug")
    async def category(self, slug: str) -> Optional[Category]:
        async with get_session() as session:
    ...
    async def guides(self, categorySlug: Optional[str] = None) -> List[UserGuide]:
        async with get_session() as session:
    ...
    async def guide(self, slug: str) -> Optional[UserGuide]:
        async with get_session() as session:
            result = (await session.exec(
                select(UserGuideModel).where(UserGuideModel.slug == slug)
            )).first()
...
class Mutation:
    ...
    async def submitFeedback(self, name: str, email: str, message: str, expectReply: bool) -> Feedback:
        async with get_session() as session:
