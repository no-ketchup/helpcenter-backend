...
class GuideRepository(BaseRepository[GuideModel]):
    ...
    async def list_read(self, session: AsyncSession, category_slug: Optional[str] = None) -> List[GuideReadDTO]:
    ...
    async def get_categories(
        ...
        stmt = (
            ...
            .where(GuideCategoryLink.guide_id == guide_id)
        )
        result = await session.execute(stmt)
    ...
    async def delete(self, session: AsyncSession, id: UUID) -> bool:
        ...
        from app.domain.models import GuideCategoryLink
