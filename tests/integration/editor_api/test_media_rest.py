...
@pytest.mark.asyncio
async def test_upload_media(client, editor_headers):
    # Create a mock file
    ...
    resp = await client.post("/dev-editor/media/upload", files=files, data=data, headers=editor_headers)
...
@pytest.mark.asyncio
async def test_upload_media_with_guide_id(client, editor_headers):
    import uuid
...
@pytest.mark.asyncio
async def test_list_media(client, editor_headers):
    resp = await client.get("/dev-editor/media", headers=editor_headers)
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.asyncio
async def test_get_media_by_id(client, editor_headers):
    # First upload a media
    media_id = await test_upload_media(client, editor_headers)
    
    # Get media by ID
    resp = await client.get(f"/dev-editor/media/{media_id}", headers=editor_headers)
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.asyncio
async def test_attach_media_to_guide(client, editor_headers):
    # Create a guide
    guide_payload = {
    ...
    resp = await client.post(f"/dev-editor/guides/{guide_id}/media/{media_id}", headers=editor_headers)
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.asyncio
async def test_get_guide_media(client, editor_headers):
    import uuid
    # Create a guide
    ...
    # Get media for guide
    resp = await client.get(f"/dev-editor/guides/{guide_id}/media", headers=editor_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert isinstance(data, list)
...
@pytest.mark.asyncio
async def test_detach_media_from_guide(client, editor_headers):
    # Create a guide
    guide_payload = {
    ...
    media_id = await test_upload_media_with_guide_id(client, editor_headers)
    
    # Detach media from guide
    resp = await client.delete(f"/dev-editor/guides/{guide_id}/media/{media_id}", headers=editor_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert "message" in data
...
@pytest.mark.asyncio
async def test_delete_media(client, editor_headers):
    # Upload media
    media_id = await test_upload_media(client, editor_headers)
    
    # Delete media
    resp = await client.delete(f"/dev-editor/media/{media_id}", headers=editor_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert "message" in data
    
    # Verify it's deleted
    resp2 = await client.get(f"/dev-editor/media/{media_id}", headers=editor_headers)
    assert resp2.status_code == 404


@pytest.mark.asyncio
async def test_upload_invalid_file_type(client, editor_headers):
