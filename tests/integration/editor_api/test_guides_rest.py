...
async def test_create_guide_and_fetch_by_id(editor_client, editor_headers):
    ...
    # Create
    resp = await editor_client.post("/dev-editor/guides", json=payload, headers=editor_headers)
    ...
    # Fetch by ID
    resp2 = await editor_client.get(f"/dev-editor/guides/{guide_id}", headers=editor_headers)
...
async def test_create_duplicate_slug_conflict(editor_client, editor_headers):
    ...
    resp1 = await editor_client.post("/dev-editor/guides", json=payload, headers=editor_headers)
    assert resp1.status_code == 200

    # Second create should conflict
    resp2 = await editor_client.post("/dev-editor/guides", json=payload, headers=editor_headers)
...
async def test_list_and_fetch_by_slug(editor_client, editor_headers):
    ...
    resp = await editor_client.post("/dev-editor/guides", json=create_payload, headers=editor_headers)
    assert resp.status_code == 200

    # Should list the created guide
    resp = await editor_client.get("/dev-editor/guides", headers=editor_headers)
    assert resp.status_code == 200
    items = resp.json()
    ...
    # Fetch by slug
    resp2 = await editor_client.get(f"/dev-editor/guides/slug/{unique_slug}", headers=editor_headers)
    assert resp2.status_code == 200
    data = resp2.json()
    assert data["slug"] == unique_slug
...
async def test_update_guide(editor_client, editor_headers):
    ...
    resp2 = await editor_client.put(f"/dev-editor/guides/{guide_id}", json=update_payload, headers=editor_headers)
    assert resp2.status_code == 200
    updated = resp2.json()
...
async def test_delete_guide(editor_client, editor_headers):
    ...
    # Delete the guide
    resp2 = await editor_client.delete(f"/dev-editor/guides/{guide_id}", headers=editor_headers)
    assert resp2.status_code == 200

    # Verify it's deleted
    resp3 = await editor_client.get(f"/dev-editor/guides/{guide_id}", headers=editor_headers)
